...
void create_tar(const char *source_dir, const char *tar_file, const char *file_type) {
    ...
    snprintf(command, sizeof(command), "find %s/stext -name '*.%s' | xargs tar -cvf %s", source_dir, file_type, tar_file);
       fprintf(stderr,command,strlen(command));
    system(command);
}
//Function to create the directories as done in smain for TEXT files
...
void process_client(int client_socket) {
    ...
    if ((bytes_read = read(client_socket, buffer, 200)) > 0) {
        ...
        if (strncmp(buffer, "ufile", 5) == 0) {
            ...
            snprintf(full_path, sizeof(full_path), "%s/stext/%s/%s", home_dir, destination, filename);
            snprintf(full_dir_path, sizeof(full_dir_path), "%s/stext/%s", home_dir, destination);
            //
            fprintf(stderr,"The final path is %s ", full_path);
            ...
            //finally printing the full path
            ...
            write(client_socket, "File stored in stext server.", strlen("File stored in stext server."));
            ...
            write(client_socket, "File removed from Stext server.", strlen("File removed from Stext server."));
        
        } else if (strncmp(buffer, "dtar", 4) == 0) {
            // File name is filetype
            sscanf(buffer, "dtar %s", filename);
            
            snprintf(full_path, sizeof(full_path), "%s/stext/text.tar", home_dir);
            create_tar(home_dir, full_path, "txt");
            send_file(client_socket, full_path);
        }
        else {
...
void send_file(int socket, const char *filename) {
    ...
    while ((bytes_read = read(file_fd, buffer, BUFFER_SIZE)) > 0) {
        //Writing into the socket the bytes read
...
//Listening
    ...
    printf("Stext server listening on port %s\n", argv[1]);
